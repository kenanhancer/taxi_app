...
def lambda_handler(event, context):
    try:
        ...
        ride_request_item_result = build_ride_request_dynamodb_item(body, ride_id)

        if ride_request_item_result.status_code != 200:
            return ride_request_item_result

        # Store the ride request in DynamoDB
        ride_request_store_result = store_ride_request_in_dynamodb(
            ride_request_item_result.data
        )
